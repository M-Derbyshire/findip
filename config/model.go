...
type Config struct {
	...
	Port               uint16 `description:"The port to send requests to during the scan"`
	Endpoint           string `description:"The endpoint to send requests to during the scan"`
	ConcurrentRequests int    `description:"How many concurrent HTTP requests should be sent out at once?"`
	TextToFind         string `description:"The text that we want to find in a HTTP response body"`
}

func CreateConfigWithDefaults() Config {
	return Config{
		FromIp:             []byte{192, 168, 0, 1},
		ToIp:               []byte{192, 168, 0, 100},
		Port:               80,
		Endpoint:           "",
